...
// 获取即将开始的秒杀活动
app.get("/activities/upcoming", async (c) => {
  try {
...
app.post(
  "/activities/:id/join",
  authMiddleware,
  createRateLimit(rateLimitConfigs.seckill),
  ...
  async (c) => {
    try {
      ...
      const result = await joinSeckill(user.userId, activityId, quantity);
      
      return c.json({
        success: true,
        data: result,
        message: "秒杀下单成功，请在15分钟内完成支付",
      });
    } catch (error: any) {
